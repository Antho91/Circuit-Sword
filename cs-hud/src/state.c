...
if (gpioInitialise() < 0) {
  ...
  if (c.gpio_pin_overtemp > -1) {
    gpioSetMode(c.gpio_pin_overtemp, PI_OUTPUT);
    gpioWrite(c.gpio_pin_overtemp, 0);
  }

  if (c.gpio_pin_wifi > -1) {
    gpioSetMode(c.gpio_pin_wifi, PI_OUTPUT);
    gpioWrite(c.gpio_pin_wifi, 1);
  }
  ...
  if (c.setting_input == INPUT_GPIO) {
    ...
    int gpio_in_pins[16] = { 
        c.gpio_in_up, c.gpio_in_down, c.gpio_in_left, c.gpio_in_right,
        c.gpio_in_start, c.gpio_in_select, c.gpio_in_a, c.gpio_in_b,
        c.gpio_in_x, c.gpio_in_y, c.gpio_in_c1, c.gpio_in_c2,
        c.gpio_in_l1, c.gpio_in_l2, c.gpio_in_r1, c.gpio_in_r2
    };

    for (int i = 0; i < 16; i++) {
        if (gpio_in_pins[i] > -1) {
            gpioSetMode(gpio_in_pins[i], PI_INPUT);
            gpioSetPullUpDown(gpio_in_pins[i], PI_PUD_UP); // optioneel
        }
    }
}
...
void process_temperature()
{
    ...
    if (cs_state.temperature > cs_state.temperature_threshold) {
        if (!cs_state.temperature_over) {
            if (c.gpio_pin_overtemp > -1) {
                printf("[d] FAN ON\n");
                gpioWrite(c.gpio_pin_overtemp, 0); // pigpio
            }
        ...
        if (cs_state.temperature_over) {
            if (cs_state.temperature < cs_state.temperature_holdoff) {
                if (c.gpio_pin_overtemp > -1) {
                    printf("[d] FAN OFF\n");
                    gpioWrite(c.gpio_pin_overtemp, 1); // pigpio
                }
...
void state_process_aux_gpio()
{
  ...
  if (c.gpio_pin_pwrsw > -1) {
    if (c.setting_pwrsw_menu) {
      cs_state.mode_button_on = !gpioRead(c.gpio_pin_pwrsw);
    } else {
      cs_state.power_switch_on = gpioRead(c.gpio_pin_pwrsw);
    }
  }
  if (c.gpio_pin_chrg > -1) {
    cs_state.chrg_state = gpioRead(c.gpio_pin_chrg);
  }
  if (c.gpio_pin_pg > -1) {
    cs_state.pg_state = gpioRead(c.gpio_pin_pg);
  }
  if (c.gpio_pin_mode > -1) {
    cs_state.mode_button_on = !gpioRead(c.gpio_pin_mode);
  }
