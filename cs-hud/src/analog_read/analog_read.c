...
double read_analog_voltage_average() {
  ...
  for (uint8_t pos = 0; pos < voltage_average_count; pos++) {
...
void read_analog_start()
{
  if (!voltage_pin_ready) {
    ...
    gpioSetMode(pin_voltage, PI_OUTPUT);
    gpioWrite(pin_voltage, 0);
    voltage_pin_ready = 1;
  }
  ...
  // Set pin to read
  gpioSetMode(pin_voltage, PI_INPUT);
  voltage_available = 0;
}
...
bool read_analog_process()
{
  if (voltage_available) {
    // Calc us elapsed
    double elapsed = (((et.tv_sec - st.tv_sec) * 1000000) + 
                     (et.tv_usec - st.tv_usec)) / 1000.0;

    // Convert to voltage
    double voltage = - ( config_val_vc /
        (exp(-((elapsed))/(config_val_r*config_val_c)) - 1.0) );
...
// Callback function for when the pin goes high (pigpio style)
void read_analog_callback(int gpio, int level, uint32_t tick)
{
  if (level == PI_HIGH && voltage_pin_ready) {
    voltage_pin_ready = 0;
...
void read_analog_init(int pin)
{
  ...
  gpioSetMode(pin_voltage, PI_OUTPUT);
  gpioWrite(pin_voltage, 0);
  gpioDelay(200 * 1000); // 200ms

  // Register callback (pigpio alert)
  gpioSetAlertFunc(pin_voltage, read_analog_callback);
}
